...
describe('Contract Service', () => {
  ...
  beforeEach(() => {
    ...
    mockHyperDex = {
      ...
      relayer: jest.fn().mockResolvedValue(TEST_ACCOUNTS.relayer.address),
      swapExactInputSingleViaRelayer: jest.fn().mockResolvedValue({
    ...
    mockFactory = {
      ...
      getPool: jest.fn().mockResolvedValue('0x9A676e781A523b5d0C0e43731313A708CB607508')
    };
    
    // Mock ethers Contract constructor
    jest.spyOn(ethers, 'Contract').mockImplementation((address, abi, signerOrProvider) => {
      if (abi === HyperDexABI) {
        return mockHyperDex;
  ...
  test('initializes with correct contract instances', () => {
    expect(contractService).toBeDefined();
    expect(contractService.contracts.hyperDex).toBeDefined();
    expect(contractService.contracts.factory).toBeDefined();
  });

  test('getPool loads and caches pool instances', async () => {
    ...
    expect(mockFactory.getPool).toHaveBeenCalledWith(tokenA, tokenB, fee);
    
    // Verify pool was created and cached
    expect(pool).toBeDefined();
    expect(pool.address).toBe('0x9A676e781A523b5d0C0e43731313A708CB607508');
    expect(contractService.poolCache.size).toBe(1);
    ...
    expect(mockFactory.getPool).toHaveBeenCalledTimes(1);
  });

  test('executeGaslessSwap correctly proxies the swap to HyperDex contract', async () => {
    ...
    expect(contractService.contracts.hyperDex.swapExactInputSingleViaRelayer).toHaveBeenCalledWith(
      swapParams, 
      signature,
      {}
    );
  ...
  test('executes direct pool swap for pools that support it', async () => {
    ...
    expect(pool.gaslessSwap).toHaveBeenCalledWith(
      swapParams,
      signature,
      {}
    );
    ...
    expect(result[0].toString()).toBe(ethers.utils.parseEther('1').toString());
    expect(result[1].toString()).toBe(ethers.utils.parseEther('-0.9').toString());
  });

  test('throws error for invalid contract addresses', async () => {
  ...
  test('handles non-existent pools correctly', async () => {
    // Mock factory.getPool to return zero address (non-existent pool)
    mockFactory.getPool.mockResolvedValueOnce(ethers.constants.AddressZero);
